...
import sys
sys.path.append('../../../')
from metdig.metdig_graphics.cmap.cm import make_cmap
...
def cm_rain_nws(atime=24, pos=None):
    ...
    if pos is None:
        ...
        _pos = pos
    return mpl.colors.ListedColormap(_colors)
...
def cm_temperature_nws(pos=None):
    ...
    if pos is None:
        ...
        _pos = pos
    return make_cmap(_colors, position=_pos, rgb=True), _pos
...
def cm_cloud_cover_nws(pos=None):
    ...
    return make_cmap(_colors, position=_pos, hex=True), _pos
...
def cm_precipitable_water_nws(pos=None):
    ...
    if pos is not None:
        ...
        _pos = np.concatenate((np.arange(25), np.arange(26, 84, 2)))
...
def cm_specific_humidity_nws(pos=None):
    ...
    if pos is not None:
        ...
        _pos = [2, 4, 6, 8, 10, 12, 14, 16, 20, 22, 24]
...
def cm_wsp(pos=None):
    _colors = ["#FFF59D", "#FFEE58", "#FFCA28", "#FFC107", "#FF9800", "#FB8C00", '#E64A19', '#BF360C']  # RRGGBBAA
    cmap = mpl.colors.ListedColormap(_colors)
    return cmap


def cm_rain(pos=None):
    _colors = ["#88F492", "#00A929", "#2AB8FF", "#1202FC", "#FF04F4", "#850C3E"]
    cmap = mpl.colors.ListedColormap(_colors)
    return cmap
    ...
    for key in func:
        if key.startswith('cm'):
            ...
            try:
                ...
                try:
                    colors = cmap(range(256))[:, :3]
                except Exception as e:
                    raise e


            rgb = list(map(lambda x: torgb(x), colors))
            rgb = np.array(rgb)
            print(key, rgb.dtype)
            fdir = '../resources/colormaps_met'
            if not os.path.exists(fdir):
                os.makedirs(fdir)
            fname = os.path.join(fdir, key.strip('cm_') + '.rgb')
            header = '#       r           g          b'
            if rgb.dtype == 'int32':
                np.savetxt(fname, rgb, fmt='%10d', header=header, comments='')
            else:
                np.savetxt(fname, rgb, fmt='%10.6f', header=header, comments='')
