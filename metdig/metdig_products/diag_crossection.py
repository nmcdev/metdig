...
def draw_wind_theta_mpv(cross_mpv, cross_theta, cross_u, cross_v, cross_terrain, hgt,
                         ...
                         mpv_contour_kwargs={},  uv_barbs_kwargs={}, theta_contour_kwargs={}, hgt_contour_kwargs={}):
    ...
    # plot absv
    _x = cross_mpv['lon'].values
    _y = cross_mpv['level'].values
    _z = cross_mpv.values.squeeze()
    mpv_contour = draw_method.cross_mpv_contourf(ax, _x, _y, _z,**mpv_contour_kwargs)
    # plot colorbar
    l, b, w, h = ax.get_position().bounds
    cax = plt.axes([l + 0.01 + w, b, 0.015, h])
    cb = plt.colorbar(mpv_contour, cax=cax, orientation='vertical', extend='max', extendrect=False)
    cb.set_label('Moisture Potential Vorticity (10$^{-6}$ K*m**2/(s*kg))')

    # plot theta
    _x = cross_theta['lon'].values
    _y = cross_theta['level'].values
    _z = cross_theta.values.squeeze()
    theta_contour = ax.contour(_x, _y, _z, levels=np.arange(250, 450, 5), colors='k', linewidths=2,**theta_contour_kwargs)
    ...
    ax.barbs(cross_u['lon'][wind_slc_horz].values,
             ...
             cross_v.values.squeeze()[wind_slc_vert, wind_slc_horz] * 2.5, color='k',**uv_barbs_kwargs)
    ...
    draw_method.cross_section_hgt(_x, _y, _z, levels=np.arange(500, 600, 4), cmap='inferno',
                                  ...
                                  map_extent=map_extent, h_pos=h_pos,**hgt_contour_kwargs)
