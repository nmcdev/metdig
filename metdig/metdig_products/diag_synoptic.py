...
def draw_pv_div_uv(pv, div, u, v, map_extent=(60, 145, 15, 55), **products_kwargs):
    ...
    obj = horizontal_compose(title=title, description=forcast_info, png_name=png_name, map_extent=map_extent, **products_kwargs)
    div_contourf(obj.ax, div, levels=np.arange(-10, 11, 1), cmap='PuOr', extend='both', alpha=0.5)
    uv_barbs(obj.ax, u, v)
    pv_contour(obj.ax, pv)
    return obj.save()
